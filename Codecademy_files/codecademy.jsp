		...
		get_impact_pixel: function () {
			...
			try {
				for (var i = 0; i < document.getElementsByTagName("script").length; i++) {
					var text = document.getElementsByTagName("script")[i].innerText;
					if (text.indexOf("ire('trackConversion'") != -1) {
						pixel = text.trim().replace(/ /g, '').replace(/\t/g, '').replace(/\n/g, '');
						pixel = pixel.split("ire('trackConversion'")[1];
						pixel = pixel.split("});")[0];
						return pixel;
					}
				}
